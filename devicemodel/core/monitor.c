...
static void handle_diskplug(struct mngr_msg *msg, int client_fd, void *param)
{
	struct mngr_msg ack;
	struct vm_ops *ops;
	int ret = 0;
	...
	LIST_FOREACH(ops, &vm_ops_head, list) {
		if (ops->ops->diskplug) {
			ret += ops->ops->diskplug(ops->arg, msg->data.devargs);
			count++;
		}
	}
...
int monitor_init(struct vmctx *ctx)
{
	...
	ret += mngr_add_handler(monitor_fd, DM_DISKPLUG, handle_diskplug, NULL);
